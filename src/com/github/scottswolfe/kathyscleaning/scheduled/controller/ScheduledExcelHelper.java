...
public class ScheduledExcelHelper implements ExcelHelper<NW_Data> {
    ...
    private static final String SCHEDULED_SHEET_NAME = "PRINTOUTS";

    @Override
    public void writeModelToExcel(NW_Data model, XSSFWorkbook wb) {

        final Sheet sheet = wb.getSheet(SCHEDULED_SHEET_NAME);
    ...
    private void makeUnusedCellsBlank(XSSFWorkbook wb, final Sheet sheet) {
    ...
    private void safeMakeCellBlank(final Sheet sheet, int row, int col) {
        try {
    ...
    private Cell getCellSafely(final Sheet sheet, int r, int c, FormulaEvaluator evaluator) {
        if (sheet == null) {
            return null;
        }
